classdef Gmodel < model
    % Gaussian model, inherits from class model. Model only consists of multiplets of Gauss-curves.

    properties
        mname = 'Gaussmodel'        % Predefined name of model - don't change
    ...
    methods
        %% Constructor
        function obj = Gmodel(Tfull, init, lower, upper, act, Tsidx)
            obj.Tfull = Tfull;
        ...
        function redmod = getReducedModel(obj, redTsidxIdx)
            ...
            redmod = Gmodel(obj.Tfull(min(redTsidx):max(redTsidx)));
        ...
        function interpPara(obj, status, xmin, xmax, changed)
            ...
            for i=1:obj.lenPeak
                for k = 2:length(obj.ParaName) % No need to change 'amt
                    eval(['actflag = obj.act.' obj.ParaName{k} '(i,idxint);']) % dD we need to interpolate (only relevant for split optimization)
        ...
        function obj = initializeConst(obj, initLayer, Tsidx, options, ma, X)
            ...
            for i=1:obj.lenPeak
                if obj.init.amt(i,1) == 1
                    obj.init.dist(i,:) = -1;
                    obj.act.dist(i,:) = false; % Don't optimize distance if there's none to optimize.
                end
            end
            ...
            for i=1:lenTs
                obj.lower.center(:,i) = min(X{Tsidx(i)});   % Constrains centers
        ...
        function initLayer = analyzeFirstLayer(obj, params, Xinit, Dinit, options, spath, BC)
            % After finding all multiplets' centeridx & amt values and suggested distances (in indexes) (params), optimize dist, hw, height
            L1opt = options.L1opt;
            bw = options.bwdth;
            ...
            initLayer.center    = initLayer.center(:); % Produces a column vector, that is indeed important
            ...
            % Optimize 'dist', 'hw', 'height' at once, first for each peak respectively
            ...
            for i = 1:obj.lenPeak
                ...
                if amt == 1
                    smol_fun = @ (para) (Dinit(lb:rb) -glmult(Xinit(lb:rb), amt, center, -1, para(1), para(2), 1, BC)); % Errorfunction is difference from data to model
                    ...
                    % lambda = 1 for Gaussian model
                    smol_fun = @ (para) (Dinit(lb:rb) -glmult(Xinit(lb:rb), amt, center, para(1), para(2), para(3), 1, BC)); % Errorfunction is difference from data to model
            ...
            if options.info >=2
                ...
                hold on
                for i=1:obj.lenPeak
                    plot(Xinit, glmult(Xinit, initLayer.amt(i), initLayer.center(i), initLayer.dist(i), initLayer.hw(i), initLayer.height(i), 1, BC), 'r--')
            ...
            cact.dist        = true(size(initLayer.dist));

            clower.hw        = zeros(size(initLayer.hw));
            cupper.hw        = L1opt.maxhw*ones(size(initLayer.hw));
            cact.hw          = true(size(initLayer.hw)); % Only temporarily fully active
            ...
            model = Gmodel(obj.Tfull(1), initLayer, clower, cupper, cact, 1); % Creates auxiliary model of a single time layer to optimize
        ...
        function Y = MOD(obj, para, X, BC) % Output in cell-format Y{i}(j) has the datapoint j in timelayer i
            para = obj.scaleM*para; % 'para' is passed after scaling

            paraR = reshape(para,obj.lenPeak, length(obj.Ts), length(obj.ParaName)); % Shape matters, makes parameters easily accessible
            ...
            if length(obj.Ts) == 1 % Handles differently, doesn't need spline interpolation
                ...
                Y{1} = zeros(size(X{1}));
                for i=1:obj.lenPeak
                    Y{1} = Y{1} + glmult(X{1}, amtfull(i,1), centerfull(i,1), distfull(i,1), hwfull(i,1), heightfull(i,1), 1, BC);
                end
            else
                for i=1:obj.lenPeak
                ...
                for j=1:length(obj.Tfull)
                    Y{j} = zeros(size(X{j}));
                    for i=1:obj.lenPeak
                        Y{j} = Y{j} + glmult(X{j}, amtfull(i,j), centerfull(i,j), distfull(i,j), hwfull(i,j), heightfull(i,j), 1, BC);
                    end
                end
            end
        end
        ...
        % where 'xyz_i,j' is the value of the 'i'-th multiplet parameter 
        % 'xyz' in layer 'j'
        ...
        function obj = showpara(obj)
            ...
            uf.CloseRequestFcn=@(src,call) my_closereq(src,call,uf,uit,tf); % Function located in \bin folder
        ...
        function comptime = optimizeCenters(obj, opt, X, CP)
            ...
            actind = obj.act.dist(:);
            % Everything needs to be vector for lsqnonlin
        ...
        function boundmod = getBoundedModel(obj, redTsidxIdx, multidx)
            ...
            boundmod = Gmodel(obj.Tfull(min(redTsidx):max(redTsidx)));
        ...
        function obj = analyzeSingLayer(obj, Xinit, Dinit, specidx, options, BC)
            ...
            C = zeros(obj.lenPeak,1);
            for i=1:obj.lenPeak
                distinit(i) = sdist(i); % sSt for all peaks, only used for true multiplets
            ...
            for i=1:obj.lenPeak % Eee, i told you not to worry
            ...
            model = Gmodel(obj.Tfull(1), inits, clower, cupper, cact, 1); % Creates auxiliary model of a single time layer to optimize
